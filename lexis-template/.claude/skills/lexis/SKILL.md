...
description: >
  ...
  knitr/fragment gotchas specific to this format. Includes drop-in scaffolds for
  common layout requests ("make a two-column slide"). Also porting
  xaringan/remark `.Rmd` decks (the original lexis theme) to lexis-revealjs.
metadata:
...
- **Sizes:** `.small` `.large`, or `.font10` … `.font200` (5% steps) for an
  exact percentage. This is the one sizing system for *all* content: as a span
  for inline text, or as a fenced div (`::: {.font70}`) around paragraphs,
  lists, or code chunks — a wrapped chunk and its printed output scale
  together. (There is no separate `.codeNN` system; xaringan-era `.codeNN`
  classes must be converted to `.fontNN` when porting old decks.)
...
class**. This applies everywhere, but shows up most with columns:

```markdown
<!-- verbose — don't do this -->      <!-- write this instead -->
::: {.col}                            ::: {.col .font60 .center}
::: {.font60}                         content
::: {.center}                         :::
content
:::
...
Same for spans: `[text]{.red .font80}`. Two rules that silently break this:
every class needs its own leading dot (`{.col font60}` drops `font60`), and
key–value attributes like `width="30%"` mix freely with classes on the same
fence (`::: {.col .font60 width="30%"}`).

## Scaffolds for common requests

When asked to "add/make a … slide," insert the matching scaffold as a **new
`---` slide** and leave the bodies blank (or drop in only content the user
supplied) — don't invent filler text. Scaffolds already follow the
one-div-many-classes rule; extra classes go on the existing fence.

**Two-column slide** ("two columns", "side by side"):
...
For an uneven split, put `width` on one column: `::: {.col width="65%"}`. For
three or four columns, just add more `.col` divs — the row splits evenly.

**Section divider** ("section break", "divider slide"):
...
**Image beside text** ("image on the right"):
...
To audit an existing deck for errors, legacy syntax, and cleanup opportunities,
use the companion `lexis-clean` skill (`/lexis-clean`).
